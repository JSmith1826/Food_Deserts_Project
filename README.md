...
We then created a SQL database by importing each csv sheet into postgres/pgAdmin as a seperate table. 
...
We also needed to furfil a requirement that our data be hosted in the cloud. For this task we chose to create an remote database on Amazon Web Services and connect it to out local database
...
We created a website using Canva.
